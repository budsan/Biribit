...
macro(sys_add_library target)
    ...
    if(BUILD_SHARED_LIBS)
        ...
        if (SYS_OS_WINDOWS AND SYS_COMPILER_GCC)
    ...
    if(NOT SYS_OS_ANDROID)
        set_target_properties(${target} PROPERTIES SOVERSION ${VERSION_MAJOR}.${VERSION_MINOR})
        set_target_properties(${target} PROPERTIES VERSION ${VERSION_MAJOR}.${VERSION_MINOR}.${VERSION_PATCH})
    endif()

    # set the target's folder (for IDEs that support it, e.g. Visual Studio)
    set_target_properties(${target} PROPERTIES FOLDER "SYS")

    # for gcc >= 4.0 on Windows, apply the SYS_USE_STATIC_STD_LIBS option if it is enabled
    if(SYS_OS_WINDOWS AND SYS_COMPILER_GCC AND NOT SYS_GCC_VERSION VERSION_LESS "4")
        if(SYS_USE_STATIC_STD_LIBS AND NOT SYS_COMPILER_GCC_TDM)
            set_target_properties(${target} PROPERTIES LINK_FLAGS "-static-libgcc -static-libstdc++")
        elseif(NOT SYS_USE_STATIC_STD_LIBS AND SYS_COMPILER_GCC_TDM)
    ...
    if(NOT SYS_OS_WINDOWS AND ((SYS_COMPILER_GCC AND NOT SYS_GCC_VERSION VERSION_LESS "4") OR (SYS_COMPILER_CLANG AND NOT SYS_CLANG_VERSION VERSION_LESS "3")))
        set_target_properties(${target} PROPERTIES COMPILE_FLAGS -fvisibility=hidden)
    endif()

    # link the target to its SYS dependencies
    if(THIS_DEPENDS)
        target_link_libraries(${target} ${THIS_DEPENDS})
    endif()

    # build frameworks or dylibs
    if(SYS_OS_MACOSX AND BUILD_SHARED_LIBS)
        if(SYS_BUILD_FRAMEWORKS)
            # adapt target to build frameworks instead of dylibs
            set_target_properties(${target} PROPERTIES
                                  FRAMEWORK TRUE
                                  FRAMEWORK_VERSION ${VERSION_MAJOR}.${VERSION_MINOR}.${VERSION_PATCH}
                                  MACOSX_FRAMEWORK_IDENTIFIER org.sys-dev.${target}
    ...
    # sys-activity library is our bootstrap activity and must not depend on stlport_shared
    # (otherwise Android will fail to load it)
    if (SYS_OS_ANDROID)
        if (${target} MATCHES "sys-activity")
...
# add a new target which is a SYS example
# ex: sys_add_example(ftp
#                      SOURCES ftp.cpp ...
#                      DEPENDS sys-network sys-system)
macro(sys_add_example target)
    ...
    # create the target
    if(THIS_GUI_APP AND SYS_OS_WINDOWS AND NOT DEFINED CMAKE_CONFIGURATION_TYPES AND ${CMAKE_BUILD_TYPE} STREQUAL "Release")
        add_executable(${target} WIN32 ${THIS_SOURCES})
        target_link_libraries(${target} sys-main)
    else()
